          ...
          <tbody class="bg-white divide-y dark:divide-gray-700 dark:bg-gray-800">
            ...
            <tr class="text-gray-700 dark:text-gray-400 ">
              ...
              <td class="px-4 py-3 text-xs">
                <span
                  class="px-2 py-1 font-semibold lh-sm text-red-700 bg-red-100 rounded-md dark:text-red-100 dark:bg-red-700">
                  {{ $item->status }}
                </span>
              </td>
...
@section('content')
    ...
    <main id="main" class="main">
        ...
        <div class="pagetitle my-3">
            <h1>Pengaduan</h1>
            <p>Data dari seluruh pengaduan</p>
            @if ($errors->any())
                <div class="alert alert-danger">
                    <ul>
                        @foreach ($errors->all() as $error)
        ...
        <section class="section dashboard my-4">
            <div class="table-responsive row bg-light shadow rounded p-3">
                <table class="table table-bordered">
                    <thead>
                        <tr class="text-secondary text-opacity-75">
                    ...
                    <tbody class="">
                        @forelse ($items as $item)
                            <tr>
                                {{-- <td> <img class="w-" src="{{ Storage::url($item->image) }}" alt=""
                                        loading="lazy" /></td> --}}
                                <td class="text-capitalize">{{ $item->name }}</td>
                                <td>{{ $item->created_at->format('l, d F Y - H:i:s') }}</td>
                                <td class="text-center">
                                    @if ($item->status == 'Belum di Proses')
                                        <span href="" class="btn btn-danger btn-sm">{{ $item->status }}</span>
                                    @elseif ($item->status == 'Sedang di Proses')
                                        <span href="" class="btn btn-warning btn-sm">{{ $item->status }}</span>
                                    @else
                                        <span href="" class="btn btn-success btn-sm">{{ $item->status }}</span>
                                    @endif
                                </td>
                                        ...
                                        <div class="modal-dialog modal-dialog-centered">
                                            <div class="modal-content">
                                                <div class="modal-header">
                                                    <h4 class="modal-title fs-5" id="exampleModalLabel">Yakin Hapus?</h4>
                                                    <button type="button" class="btn-close" data-bs-dismiss="modal"
                                                        aria-label="Close"></button>
                                                </div>
                                                <div class="modal-body">
                                                    Kamu yakin akan menghapus aduan dari <span
                                                        class="text-capitalize">{{ $item->name }}</span> pada
                                                    <span>{{ $item->created_at->format('l, d F Y - H:i:s') }}</span>
                                                </div>
                                                <div class="modal-footer">
                                                    <button type="button" class="btn btn-secondary"
                                                        data-bs-dismiss="modal">Batal</button>
                                                    <form action="{{ route('pengaduans.destroy', $item->id) }}"
                                                        method="POST">
                                                        @csrf
                                                        @method('delete')
